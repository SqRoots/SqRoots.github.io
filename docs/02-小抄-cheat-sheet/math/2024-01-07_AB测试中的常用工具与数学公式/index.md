...
$$
\dfrac{\overline{x}-\mu}{\sqrt{\sigma^2/n}}=\dfrac{\overline{x}-\mu}{\sigma_{\overline{x}}}={\color{blue}\dfrac{\text{样本均值}-\text{总体均值}}{\text{标准误}}\sim \text{标准正态分布} N(0,1)}
$$
...
按不同需求，检验可分为**单尾检验**（$H_0:\text{一个均值大于另一个均值}$）和**双尾检验**（$H_0:\text{两总体均值相等}$）两种。常用的显著性水平有 **0.05** 和 **0.01**。
